...
- 🚀 直接内置调用 esptool，速度与命令行一致（X3 实测：备份约 2:29，写入约 1:30）
...
> ⚠️ **X4 支持说明**：使用 binwalk 分析 X4 固件后确认其分区布局与 esptool 完全兼容，因此理论支持 X4 的备份和刷机。目前仅在 X3 上完成实测，X4 尚未有实际设备验证。欢迎 X4 用户反馈测试结果。
...
仓库中还包含了预编译的 CrossPoint X3 测试版固件，位于 `firmware/crosspoint-x3-aleph/`。基于 [itsthisjustin/crosspoint-reader](https://github.com/itsthisjustin/crosspoint-reader)（x3-support 分支）编译。

- 包含 bootloader.bin、partitions.bin、firmware.bin
- 可在本工具中使用「完整写入」模式刷入。
- ⚠️ 中文功能暂未启用（字体方案待完善）。
...
本工具仅供学习研究和个人使用，以友好分享、便利刷机为宗旨免费发行。使用本工具刷写设备固件存在风险，包括但不限于设备无法启动、数据丢失、硬件异常等。使用者需自行评估风险并承担一切后果。开发者不对因使用本工具导致的任何直接或间接受损负责，且保留随时修改或停止本工具的权利。
